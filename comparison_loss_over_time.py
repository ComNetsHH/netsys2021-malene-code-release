...
def plot(filename):
	...
	with open("_data/" + filename + "__full.json") as json_file:
		...
		plt.plot(x, yAcc, label='fully obs.: accuracy', color="tab:blue", linestyle=':', linewidth=LINE_WIDTH)
		...
		plt.plot(x, yLoss, label='fully obs.: loss', color="tab:blue", linewidth=LINE_WIDTH)
	...
	with open("_data/" + filename + "__partial.json") as json_file:
		...
		yAcc_m = running_mean(yAcc_m, RUNNING_MEAN_AGGREGATE)
		yAcc_p = running_mean(yAcc_p, RUNNING_MEAN_AGGREGATE)
		plt.plot(x, yAcc, label='part. obs.: accuracy', color="tab:orange", linestyle=':', linewidth=LINE_WIDTH)
		...
		plt.plot(x, yLoss, label='part. obs.: loss', color="tab:orange", linewidth=LINE_WIDTH)
		plt.fill_between(x, yLoss_m, yLoss_p, alpha=0.5, color="tab:orange", linewidth=LINE_WIDTH)		

	fig = plt.gcf()
	fig.set_size_inches((5.8*.5, 3*.8), forward=False)
	fig.tight_layout()
	plt.legend()
	ax = plt.gca()	
...
if __name__ == '__main__':
	filename = "comparison_loss_over_time"
